...
import { and, desc, eq, like, gte, lte, inArray } from 'drizzle-orm'
import type { PostgresqlDB } from '@/infrastructure/db/connection'
import { doc, favorite } from '@/infrastructure/db/schemas/doc.pg'
...
export class PgDocRepository implements IDocRepository {
  ...
  async list(page: number, limit: number, filters?: { q?: string; source?: 'git' | 'website'; createdFrom?: number; createdTo?: number; updatedFrom?: number; updatedTo?: number }, sort?: 'popularity' | 'createdAt' | 'updatedAt'): Promise<Pagination<DocRecord>> {
    ...
    const rows = await (this.db as any).query.doc.findMany({ limit, offset, where, orderBy })
    const totalRows = await (this.db as any).query.doc.findMany({ where })
  ...
  async listFavoritesByUser(userId: number, page: number, limit: number): Promise<Pagination<DocRecord>> {
    ...
    const rows = ids.length ? await this.db.select().from(doc).where(inArray(doc.id, ids)) : []
    const content = rows.map(toDomain as any)
    const totalRows = await this.db.select().from(favorite).where(eq(favorite.userId, userId))
    const total = (totalRows as any).length
    const totalPages = Math.max(1, Math.ceil(total / limit))
    return { list:content, total, page, limit, totalPages }
  }

  async findById(id: number): Promise<DocRecord | null> {
    const row = await (this.db as any).query.doc.findFirst({ where: eq(doc.id, id) })
    return row ? toDomain(row) : null
  }

  async findBySlug(slug: string): Promise<DocRecord | null> {
    const row = await (this.db as any).query.doc.findFirst({ where: eq(doc.slug, slug) })
    return row ? toDomain(row) : null
  }
  ...
  async incrementAccess(docIdValue: number): Promise<void> {
    const existing = await (this.db as any).query.doc.findFirst({ where: eq(doc.id, docIdValue) })
    const count = ((existing as any)?.accessCount || 0) + 1
  ...
  async toggleFavorite(userId: number, docIdValue: number, like: boolean): Promise<boolean> {
    const has = await this.db.select().from(favorite).where(and(eq(favorite.userId, userId), eq(favorite.docId, docIdValue)))
    const exists = (has as any).length > 0
    if (like) {
