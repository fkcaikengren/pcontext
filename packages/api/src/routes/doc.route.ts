...
import { DocAddBodySchema, DocListQuerySchema, PositiveIntOptionalSchema } from '@/types/schemas/doc'

import { Res200, Res201, Res400, Res401, Res404, Res409 } from '../utils/response-template'
...
const router = createRouter()
  ...
  .post('/add', 
    jsonValidator(DocAddBodySchema),
    async (c) => {
  ...
  .get('/:slug', async (c) => {
    ...
    if (!doc) return c.json(Res404(null, '文档不存在') as ApiError, 404)
  ...
  .post('/:slug/favorite',
    ...
    async (c) => {
      const userId = getCurrentUserId(c)
      if (!userId) return c.json(Res401(null, '未登录') as ApiError, 401)
      const { slug } = c.req.param()
