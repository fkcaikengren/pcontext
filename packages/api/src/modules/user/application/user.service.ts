...
export async function updateUserWithValidation(id: number, input: UpdateUserDTO): Promise<UserVO | null> {
  ...
  if (patch.username !== undefined) {
    ...
    if (userByName && userByName.id !== id) {
      throw new HttpError('用户名已存在', 422)
    }
  }
