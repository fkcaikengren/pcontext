...
export interface IDocRepository {
  list: (page: number, limit: number, filters?: { q?: string; source?: 'git' | 'website'; createdFrom?: number; createdTo?: number; updatedFrom?: number; updatedTo?: number }, sort?: 'popularity' | 'createdAt' | 'updatedAt') => Promise<PaginationVO<DocEntity<Date>>>
  listFavoritesByUser: (userId: number, page: number, limit: number) => Promise<PaginationVO<DocEntity<Date>>>
